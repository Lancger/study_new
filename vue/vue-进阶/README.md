...
export default {
  ...
  computed: {
    ...
    // },
    inputValueLastLetter () {
      return this.inputValue.substr(-1, 1)    ---通过计算属性，获取inputValue输入的最后一个字母（这里是局部的geters,不是根级别的geters）
    }
  },
  methods: {
    handleInput (val) {
...
export default {
  ...
  computed: {
    ...
    ...mapGetters([        ---或者使用mapGetters方式获取
      'appNameWithVersion',
      'firstLetter'
    ]),
    
    appNameWithVersion () {
      return this.$store.getters.appNameWithVersion     ----使用这种方式获取
    },
